...
| Frontend | React (Vite) + TypeScript + Tailwind, with TanStack Query/Table and Radix primitives | Chosen over a full component kit because the directory's paging, sorting and filtering are all server-driven, and a headless table keeps that contract explicit rather than adapting a component with its own opinions about state. Radix supplies accessible dropdown and dialog behaviour without imposing a visual style. |
